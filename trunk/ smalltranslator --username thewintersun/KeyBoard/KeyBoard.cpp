...
{ 
	...
	if(nCode==HC_ACTION) 
	{
		...
		case WM_SYSKEYDOWN:
			...
			if((p->vkCode==162)&&(keydown==0)) //160是shift键的 ,164是alt键 ，162是ctrl键
			{		
				...
				if (h_Highlight_Wind!=NULL) 
				{
					...
					SendMessage(h_globalWind,WM_MYMSG,0,0);
				}
			}
				break;
			...
			case WM_SYSKEYUP:
				...
				if(p->vkCode==162) //160是shift键的，164是alt键 ，162是ctrl键
				{
					TRACE("key up :%d",p->vkCode);
					keydown=0;
					TRACE("GET shite up message ....");
					SendMessage(h_globalWind,WM_MYMSG_DESTROY,0,0);
				}
		default:;
		}
	} 
